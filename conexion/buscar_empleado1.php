<?php 
	...
	$salida="";
	$query="SELECT * FROM empleados,personas WHERE personas.Id_persona=empleados.Id_persona";
	if (isset($_POST['consulta'])) {
		$palabra=$conexion->real_escape_string($_POST['consulta']);
		$query="SELECT * FROM empleados,personas WHERE personas.Id_persona=empleados.Id_persona AND  Nombre LIKE '".$palabra."%'";
	}
		  ...
		  while ($fila=$resultado->fetch_assoc()) {
		  	$salida.="
		        ...
		        <tr class='text-center'>
		        	<td><small>".$fila['Id_empleado']."</small></td>
					...
					<td><small>".$fila['Fecha_contratacion']."</small></td>
		        </tr>";
		  }
		  $salida.="
		  		</tbody>
